...
from .utils import (
    null_or, stanox_to_station, decode_timestamp, decode_bool
)
...
class TrainMovementParser(object):
    ...
    def parse(self):
        return {
            ...
            'location_stanox': self.body['loc_stanox'],

            'location_station': null_or(
                stanox_to_station, self.body['loc_stanox']),

            'train_file_address': self.body['train_file_address'],
            ...
            'next_report_stanox': self.body['next_report_stanox'],

            'next_report_station': null_or(
                stanox_to_station, self.body['next_report_stanox']),

            'train_service_code': self.body['train_service_code'],

            'variation_status': self.body['variation_status'],

            # current_train_entity=self.body['current_train_id'], #  TODO

            'event_type': self.body['event_type'],

            'next_report_run_time': null_or(
                int, self.body['next_report_run_time']),
            ...
            'planned_event_type': self.body['planned_event_type'],
